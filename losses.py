...
class compute_loss(nn.Module):
    ...
    def forward(self, pred_list, coord_list, gt_norm, gt_norm_mask):
        return self.loss_fn(pred_list, coord_list, gt_norm, gt_norm_mask)
    ...
    def forward_UG(self, pred_list, coord_list, gt_norm, gt_norm_mask):
        ...
        for (pred, coord) in zip(pred_list, coord_list):
            if coord is None:
                ...
                # coord: B, 1, N, 2
                # pred: B, 4, N
                gt_norm_ = F.grid_sample(gt_norm, coord.double(), mode='nearest', align_corners=True)  # (B, 3, 1, N)
                gt_norm_mask_ = F.grid_sample(gt_norm_mask.float(), coord.float(), mode='nearest', align_corners=True)  # (B, 1, 1, N)
